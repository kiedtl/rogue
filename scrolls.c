...
read_scroll(void)
{
	...
	msg("As you read the prayer, it vanishes.");
	...
	break; case S_LIGHT:
		...
		if ((rp = roomin(&hero)) == NULL) {
			msg("A purplish light fills the corridor, then fades.");
		} else {
			msg("The room is lit by a shimmering white light.");
			rp->r_flags &= ~ISDARK;
			/*
	...
	break; case S_SLEEP:
		 ...
		 */
		s_know[S_SLEEP] = TRUE;
		msg("An unexplainable drowsiness envelopes you....");
	...
	break; case S_GFIND:
		...
		if (gtotal != 0) {
			s_know[S_GFIND] = TRUE;
			show_win(hw, "You begin to feel greedy and you sense gold.--More--");
		} else {
	...
	break; case S_ENCH:
		if (cur_weapon == NULL) {
			...
			if (rnd(100) > 50)
				...
				cur_weapon->o_dplus++;
			msg("A silver light envelopes your %s", w_names[cur_weapon->o_which]);
		}
	break; case S_SCARE:
		/*
	...
	break; case S_AGGR:
		 ...
		 */
		aggravate();
		msg("You feel like you've made a mistake...");
	break; case S_NOP:
		msg("You smell fried chicken in the distance.");
	break; case S_GENOCIDE:
		msg("You have been granted the boon of genocide");
		genocide();
		s_know[S_GENOCIDE] = TRUE;
	break; default:
		msg("This scroll is written in an unknown tongue.");
		return;
	}
